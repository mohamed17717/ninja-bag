...
class AccountQuerySet(models.QuerySet):
  def get_user_by_api_key(self, api_key, *, required=False):
  ...
  def get_user_acc_from_api_or_web(self, request, *, required=False):
    ...
    acc = acc or self.get_user_by_api_key(token, required=required)
...
class AccountManager(models.Manager):
  ...
  def get_user_by_api_key(self, api_key, *, required=False):
    return self.get_queryset().get_user_by_api_key(api_key, required=required)
