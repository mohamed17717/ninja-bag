...
class TextSaverModel(models.Model):
  ...
  def add(acc, text, file_name):
    ...
    TextSaverModel.objects.create(user=acc.user, file_name=file_name)
    return file_url
  ...
  def list_all(request):
    ...
    files_paths = [TextSaverModel.get_file_path(f) for f in files_names]
    files_urls = [request.build_absolute_uri(f) for f in files_paths]
